...
function animate_planar_monkeybot(x, dt)
...
#     robot_obj = mc.MeshFileGeometry(joinpath(@__DIR__,"quadrotor.obj"))
    ...
    #Three Different Colors to Differentiate the Links
    c1 = mc.RGBA(1.0, 0.0, 0.0, 1.0)
    c2 = mc.RGBA(0.0, 1.0, 0.0, 1.0)
    c3 = mc.RGBA(0.0, 0.0, 1.0, 1.0)
        
    cyp1, cyp2, r = Point(0., 0, 0), Point(3., 0, 0), 0.1
...
#     vis_traj!(vis, :traj3, X3; R = 0.01, color = c3)
    ...
    robot_arm1 = GeometryBasics.Cylinder(Point(0., 0, 0), Point(0., 0, 2), 0.1)
    robot_arm2 = GeometryBasics.Cylinder(Point(0., 0, 2), Point(0., 0, 4), 0.1)
    robot_body = GeometryBasics.Cylinder(Point(0., 0, 2), Point(0., 0, 4), 0.1)
        
    mc.setobject!(vis[:vic1], robot_arm1, mc.MeshPhongMaterial(color=c1))
    mc.setobject!(vis[:vic2], robot_arm2, mc.MeshPhongMaterial(color=c2))
    mc.setobject!(vis[:vic3], robot_body, mc.MeshPhongMaterial(color=c3))

    anim = mc.Animation(floor(Int,1/dt))
    for k = 1:length(X1)
        mc.atframe(anim, k) do
            
            update_robot_pose!(vis, :vic1, x1[k])
...
# function distance_between_quads(x1,x2,x3)
