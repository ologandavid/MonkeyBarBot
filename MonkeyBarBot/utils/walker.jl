...
function update_monkey_pose!(vis,contact_arm,model::NamedTuple,params::NamedTuple, x::Vector)
   
    theta1,theta2 = x[1]+pi/2,x[2]
    xcontact,ycontact = x[3],x[4]
    l1,l2 = params.l1,params.l2

    if contact_arm ==1 
    	xl,yl = xcontact,ycontact
    	xb = xcontact+l1*sin(theta1)
    	yb = ycontact-l1*cos(theta1) 
    	xr = xb+l2*sin(theta1+theta2)
    	yr = yb-l2*cos(theta1+theta2)
    elseif contact_arm ==0
    	xr,yr = xcontact,ycontact
    	xb = xcontact+l2*sin(theta1)
    	yb = ycontact-l2*cos(theta1) 
    	xl = xb+l1*sin(theta2+theta1)
    	yl = yb-l1*cos(theta2+theta1)
    end
